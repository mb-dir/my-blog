...
<article>
  ...
  <p>I suspect that my above explanations may not be clear enough, so I'll try to sum it up. Generally when it comes to modules we attach only one file to our .html file and everything else what we need we import to this file, we can import something to this main file if this value is export from other file - everything is connected together. When it comes to non-module approach we attach everything in our .html file and that makes that there is no relationship between parts of our code.</p>
</article>
