...
<article>
  <h2>CSS</h2>
  <p>I am not going to describe the CSS code - the truth is that you can style this HTML structre as you want, it is just my proposition(as you can see I am not design-talented). Tbh CSS is the less important in this projct. My CSS:</p>
...
<article>
  <h2>JS code</h2>
  <p>We will start with getting refference to the form and adding eventListener on submit:</p>
...
<p>Okey so we have an appropriate list item, but now we have to decide where(to which box) this item will be added. So we have to get refference to our plus/minus boxes and then we check the <q>typeSelect</q> in order to put the list item to an appropriate box.</p>
...
<p>As you can see those variables are outside the form - there is no need to get the refference to them with every form send. We have to know in which box the sum of importance is bigger in order to show a suitable communicate for user. We need to create two variable where we will keep the sum of pluses/minuses importance.</p>
...
<p>We have gotten the refference to the place where the information will be shown and based on condition with pluses and minuses sum put an appropriate comunicate to this paragraph.</p>
</article>
