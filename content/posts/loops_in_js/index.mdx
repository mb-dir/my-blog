...
<p>Another diffrence between those two ways of iterating throught array is that using for loop we can iterate only throught designated elements - using for we can easily iterate for example for 2ns element, or iterate from the end.
...
<p>Important diffrence is that <strong>forEach</strong> doesn't <q>respect</q> the <strong>break</strong> and <strong>continue</strong> key words. That means <strong>forEach</strong> goes always throught all elements in the array and we do not have the control about it.</p>

<p>So in general <strong>for</strong> loop is easier to read for begginers, it is also more <q>elastic</q>(<strong>break</strong>, <strong>continue</strong> key words and the fact that we can control the condition) and it could be used not only for iterating throught the array. <strong>forEach</strong> whereas is easier to read for more expirence programers, is more rigid, and is closely related to arrays(more specifically talking with objects containing an iterator).</p>
